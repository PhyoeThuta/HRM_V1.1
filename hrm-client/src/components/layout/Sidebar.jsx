  ...
  <div className="mb-1.5">
    ...
    >
      ...
      <div className="flex items-center gap-1">
        <svg
          className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isOpen ? 'rotate-180 text-brand-green' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth="2"
...
const ICONS = {
  ...
  birthdays: 'M21 15.546c-.523 0-1.046.151-1.5.454a2.704 2.704 0 01-3 0 2.704 2.704 0 00-3 0 2.704 2.704 0 00-3 0 2.704 2.704 0 01-1.5-.454M9 6l3-3 3 3m-3-3v12',
  ...
  orgchart: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z',
};
...
const adminRoutes = [
  '/boss', '/audit-logs', '/finance', '/announcements', '/user-accounts'
];

const isHrmRoute = (path) => hrmRoutes.some(r => path === r || path.startsWith(r + '/'));
...
export default function Sidebar({ isOpen, close }) {
  ...
  const [openCategories, setOpenCategories] = useState(() => ({
    hrm: isHrmRoute(location.pathname),
    admin: isAdminRoute(location.pathname),
  }));

  // Auto expand category accordion when route changes
  useEffect(() => {
    if (isHrmRoute(location.pathname)) {
      setOpenCategories(prev => ({ ...prev, hrm: true }));
      ...
      >
        {isEmployee() ? (
          ...
          <>
            ...
            <NavItem to="/crm/dashboard" label="CRM Dashboard" icon={ICONS.portal} />
            <NavItem to="/crm/analytics" label="Report & Analytics" icon={ICONS.boss} />

            {/* 3. Ops & Inventory (Top-level Section) */}
            ...
            {/* 4. Administration Dropdown Accordion */}
            {isAdmin() && (
              <NavCategoryAccordion
                title="Administration"
                icon={ICONS.boss}
                color="text-amber-400"
                isOpen={openCategories.admin}
                onToggle={(e) => toggleCategory('admin', e)}
              >
                ...
                <NavItem to="/announcements" label="Announcements" icon={ICONS.announcements} />
                <NavItem to="/user-accounts" label="User Accounts" icon={ICONS.users} />
