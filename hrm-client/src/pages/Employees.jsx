...
function EmployeeModal({ open, onClose, departments, positions, managers, candidates, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4" style={{ background: 'rgb(var(--color-surface-850))', border: '1px solid rgba(255,255,255,0.1)' }}>
      ...
      {tab === 'active' && (
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <input 
            ...
            className="flex-1 bg-[rgb(var(--color-surface-800))] text-slate-300 text-sm rounded-xl px-4 py-2.5 border border-white/5 outline-none focus:border-indigo-500"
          />
          <select 
            value={deptFilter} 
            onChange={(e) => setDeptFilter(e.target.value)}
            className="w-full sm:w-48 bg-[rgb(var(--color-surface-800))] text-slate-300 text-sm rounded-xl px-4 py-2.5 border border-white/5 outline-none focus:border-indigo-500"
          >
                  ...
                  sortedDepts.length > 0 ? (
                    sortedDepts.map(dept => {
                      ...
                      <React.Fragment key={dept}>
                        <tr 
                          className="cursor-pointer bg-[rgb(var(--color-surface-850))] hover:bg-white/5 transition-colors border-t border-white/5"
