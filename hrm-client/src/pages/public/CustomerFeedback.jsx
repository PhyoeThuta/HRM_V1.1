  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/public/crm/feedback', {
        ...
        body: JSON.stringify({ customer_id, rating, comment })
      });
        ...
        <div className="bg-surface-800 p-8 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="flex flex-col items-center">
              <label className="block text-sm font-bold text-slate-400 mb-4 uppercase tracking-wider">Tap to Rate</label>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <svg 
                      className={`w-12 h-12 transition-colors ${star <= (hoverRating || rating) ? 'text-amber-400 fill-amber-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.5)]' : 'text-slate-600'}`} 
                      fill="none" 
                      viewBox="0 0 24 24" 
                      stroke="currentColor"
                    >
            ...
            <div>
              <label className="block text-sm font-bold text-slate-400 mb-2">Any Comments or Suggestions?</label>
              <textarea 
                value={comment} 
