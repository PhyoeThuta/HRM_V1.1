...
export default function HandoverOutgoing() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ itemId, body }) => api.put(`/handover/${data.handover.id}/items/${itemId}`, body),
    onSuccess: () => refetch(),
    onError: (e) => toast.error(e.response?.data?.error || 'Failed to save'),
  });

  const submitMutation = useMutation({
    mutationFn: () => api.post(`/handover/${data.handover.id}/submit`),
    onSuccess: () => { refetch(); toast.success('Handover submitted for review!'); },
  ...
  const uploadMutation = useMutation({
    mutationFn: ({ file, itemId }) => {
      ...
      return api.post(`/handover/${data.handover.id}/upload`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
    },
    onSuccess: () => { refetch(); toast.success('File uploaded'); setUploadItemId(null); },
    onError: (e) => toast.error(e.response?.data?.error || 'Upload failed'),
  });

  const saveNotes = (itemId) => {
    const text = notes[itemId] ?? data?.items?.find(i => i.id === itemId)?.outgoing_notes ?? '';
  ...
  return (
    <Layout title="My Handover" subtitle={`Complete before ${handover.handover_deadline || handover.effective_date || 'your last day'}`}>
      <div className="max-w-3xl mx-auto space-y-5 pb-20">
        ...
        <div className="rounded-2xl p-5" style={{ background: '#1e2235', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div className="flex justify-between items-center mb-3">
            <div>
              <h2 className="text-base font-bold text-white">Handover Checklist</h2>
